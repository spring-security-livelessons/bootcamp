...
public class IntroApplication {
		...
		@Bean
		ApplicationRunner uuidRunner(@Value("#{ up.buildUUID() } ") String uuid,
																															@Value("#{ 2 < 1 }") boolean simpleMaths) {
		...
		public static class SimpleFilter implements javax.servlet.Filter {
				...
				public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
						...
						Assert.isTrue(request instanceof HttpServletRequest, "this must be an " + HttpServletRequest.class.getName() + ".");
